...
enum class exit_t : uint8_t { disconnect, timeout, kicked };
...
class IPlayerHandler
{
   public:
    ...
    virtual void OnPlayerQuit(IPlayer *player, exit_t exitType,
                              const char *reason)
    {
    }
    ...
    virtual void OnPlayerDamage(IPlayer *player, float healthLoss,
                                float armorLoss)
    {
    }
    virtual void OnPlayerWeaponChange(IPlayer *player, hash_t oldWeapon,
                                      hash_t newWeapon)
    {
    }

    virtual void OnPlayerRemoteEvent(IPlayer *player, uint64_t eventNameHash,
                                     const args_t &args)
    {
    }  // xxhash64

    virtual void OnPlayerStartEnterVehicle(IPlayer *player, IVehicle *vehicle,
                                           uint8_t seatId)
    {
    }
    virtual void OnPlayerEnterVehicle(IPlayer *player, IVehicle *vehicle,
                                      uint8_t seatId)
    {
    }
...
class IVehicleHandler
{
   public:
    ...
    virtual void OnVehicleDamage(IVehicle *vehicle, float bodyHealthLoss,
                                 float engineHealthLoss)
    {
    }
};
...
class ICheckpointHandler
{
   public:
    ...
    virtual void OnPlayerExitCheckpoint(IPlayer *player,
                                        ICheckpoint *checkpoint)
    {
    }
};
...
{
   public:
    virtual void OnPlayerCreateWaypoint(rage::IPlayer *player,
...
class IWorld
{
   public:
    ...
    virtual void SetWeatherTransition(const std::string &weather,
                                      float time = 0.5f) = 0;
...
class IMultiplayer
{
   public:
    ...
    virtual void AddRemoteEventHandler(const std::string &eventName,
                                       removeEventHandler_t handler) = 0;
