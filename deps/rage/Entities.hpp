...
{
struct rgb_t {
    rgb_t(uint8_t r, uint8_t g, uint8_t b) : intvalue((b << 16) + (g << 8) + r)
    {
    }

    explicit rgb_t(uint32_t val) : intvalue(val) {}
...
enum class material_t : uint8_t { Color, RGB, Paint };
...
class IPlayer : public Entity
{
   ...
   public:
    virtual void _Call(const std::string& eventName,
                       const arg_t* arguments = nullptr, size_t count = 0) = 0;
    virtual void _CallHash(uint64_t eventNameHash,
                           const arg_t* arguments = nullptr,
                           size_t count = 0) = 0;  // xxhash64
   ...
   public:
    ...
    virtual void PlayAnimation(const std::string& dict, const std::string& name,
                               float speed = 8.f, int flags = 0) = 0;
    ...
    virtual void SetClothes(
        const std::vector<std::pair<uint8_t, const clothData_t>>& clothes) = 0;

    virtual const propData_t& GetProp(uint8_t id) = 0;
    virtual void SetProp(uint8_t id, const propData_t prop) = 0;

    virtual void SetProp(
        const std::vector<std::pair<uint8_t, const propData_t>>& clothes) = 0;

    virtual void SetCustomization(
        bool gender, const rage::headBlend_t& headBlend, uint8_t eyeColor,
        uint8_t hairColor, uint8_t hightlightColor,
    ...
    virtual void SetDecorations(
        std::vector<std::pair<uint32_t, uint32_t>> decorations) = 0;
    ...
    virtual void SetHeadBlend(int shapeFirstID, int shapeSecondID,
                              int shapeThirdID, int skinFirstID,
                              int skinSecondID, int skinThirdID, float shapeMix,
                              float skinMix, float thirdMix) = 0;

    virtual void UpdateHeadBlend(float shapeMix, float skinMix,
                                 float thirdMix) = 0;
    ...
    virtual void GiveWeapons(
        std::vector<std::pair<hash_t, uint16_t>> weapons) = 0;
    ...
    virtual void RemoveObject(uint32_t model, const vector3& position,
                              float radius) = 0;
...
class IVehicle : public Entity
{
   public:
    ...
    virtual void SetColourRGB(const rage::rgb_t& primary,
                              const rage::rgb_t& secondary) = 0;
    virtual void SetColour(uint8_t primary, uint8_t secondary) = 0;
    virtual void SetPaint(const rage::paintInfo_t& primary,
                          const rage::paintInfo_t& secondary) = 0;
...
class ICheckpoint : public Entity
{
   public:
...
class IBlip : public Entity
{
   public:
    ...
    virtual void RouteFor(const std::vector<rage::IPlayer*>& players,
                          uint8_t colour, float scale) = 0;
...
class IPickup : public Entity
{
   public:
    // todo
};

class IObject : public Entity
{
   public:
};

class ITextLabel : public Entity
{
   public:
