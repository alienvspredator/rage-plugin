...
struct rgba_t {
    ...
    explicit rgba_t(uint32_t val) : intvalue(val) {}
...
struct arg_t {
   public:
    ...
    arg_t(const std::string& str)
        : type(val_t::String), v{new char[str.length() + 1]}
    {
        memcpy(v.str, str.c_str(), str.length());
        v.str[str.length()] = 0;
    }
    arg_t(entity_t entityType, entityId_t id, rage::IEntity* entity)
        : type(val_t::Entity), v{entityType, id, entity}
    {
    }
    ...
    void SetString(const std::string& str)
    {
        DeleteString();
        type = val_t::String;
        v.str = new char[str.length() + 1];
        memcpy(v.str, str.c_str(), str.length());
        v.str[str.length()] = 0;
    }
    void SetJson(const std::string& str)
    {
        DeleteString();
        type = val_t::Object;
        v.str = new char[str.length() + 1];
    ...
    void SetEntity(entity_t entityType, entityId_t id, IEntity* entity)
    {
        DeleteString();
        type = val_t::Entity;
        v.entity = {entityType, id, entity};
    }
    ...
    {
        return (type == val_t::String && v.str) ? v.str : "";
    }
    const char* Object() const
    {
        return (type == val_t::Object && v.str) ? v.str : "";
    }
    const vector3& Vector3() const
    {
        if (type == val_t::Vector3) {
            return v.vector;
        } else {
            static vector3 ret;
            return ret;
    ...
    rage::IEntity* Entity() const
    {
        return (type == val_t::Entity) ? v.entity.ptr : nullptr;
    }
    entityId_t EntityId() const
    {
        return (type == val_t::Entity) ? v.entity.id : 0xFFFF;
    }
    entity_t EntityType() const
    {
        return (type == val_t::Entity) ? v.entity.type
                                       : static_cast<entity_t>(-1);
    }

    arg_t& operator=(const arg_t& r)
    {
        DeleteString();
        if (r.GetType() != val_t::String) {
            this->v.entity = r.v.entity;
            type = r.GetType();
        } else {
            this->SetString(r.String());
        }
        return *this;
    }
    arg_t& operator=(arg_t&& r)
    {
        DeleteString();
        this->v.str = r.v.str;
        type = r.GetType();
   ...
   private:
    bool OwnsAString()
    {
        return (type == val_t::Object || type == val_t::String);
    }
    void DeleteString()
    {
        if (this->OwnsAString() && v.str) {
            delete[] v.str;
            v.str = nullptr;
        }
    }
    ...
    union _Value {
        ...
        _Value(entity_t entityType, entityId_t id, IEntity* ptr)
            : entity{entityType, id, ptr}
        {
        }
    } v;
...
struct args_t {
   public:
    args_t(arg_t* data, size_t len) : m_data(data), m_len(len) {}

    size_t Length() const { return this->m_len; }
    const arg_t& operator[](size_t id) const
    {
        if (id >= this->m_len) {
            static arg_t ar{};
            return ar;
...
class IEntity
{
   ...
   public:
    ...
    T* External()
    {
        return reinterpret_cast<T*>(this->GetExternalValue());
    }

    template <class T>
    void External(T* val)
    {
        this->SetExternalValue(reinterpret_cast<void*>(val));
    }
    ...
    virtual void SetVariables(
        const std::vector<std::pair<const std::string, const arg_t&>>&
            vars) = 0;
...
class IPool
{
   ...
   public:
    ...
    virtual std::vector<T*> GetInRange(
    ...
    class iterator
    {
       public:
        ...
        iterator(pointer ptr, uint32_t rest) : ptr_(ptr), rest_(rest - 1)
        {
            if (*ptr == nullptr)
                while (*(++ptr_) == nullptr && --rest_ != -1)
                    ;
        }
        self_type operator++()
        {
        ...
        {
            return rest_ == -1 ? false : ptr_ == rhs.ptr_;
        }
        bool operator!=(const self_type& rhs)
        {
            return rest_ == -1 ? false : ptr_ != rhs.ptr_;
